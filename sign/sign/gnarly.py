"""
X-Gnarly (ChaCha20 PRNG) implementation.

Implements ChaCha20 quarter-round, block function (8 rounds), and PRNG
initialization used by X-Bogus. Produces deterministic output from a seed.
"""
...
def get_X_Gnarly(
    ...
    add_to_object(2, 14)  # Field 2 > in version 5.1.2 always 14
    ...
    add_to_object(7, canvas_value)  # Field 7
...
def reset_prng_state():
    """Reset PRNG state (useful for testing)."""
    global prng_state, state_index
    prng_state = initialize_prng_state()
    state_index = CRYPTO_CONSTANTS[88]
