...
def sign_mobile_request(method="GET", path="", body=b"", device=None, params=None, return_headers=False):
    """دالة وسيطة لتوافق السكريبت مع مكتبة التوقيع"""
    url_str = path
    if params:
    ...
    headers = sign_all(method=method, url=url_str, body=body, device=device)
    if return_headers:
        return headers
    return headers
